...
- Tail Distribution shows GEV fit with shape parameter adjuster
- Black Swan Scenarios detail 5 extreme events with portfolio impact
- Loss Exceedance curves from 10yr to 1000yr return period
...
| Dataset | Rows | Fields |
|---|---|---|
| `BLACK_SWANS` | 6 | `name`, `probability`, `portfolioImpact`, `description`, `hedging`, `timeframe`, `severity`, `color` |
| `LOSS_EXCEEDANCE` | 8 | `loss`, `probability` |
| `SYSTEMIC_CONTRIB` | 8 | `margES`, `compES`, `systemic` |
...
| `evtFitted` | `useMemo(() => Array.from({ length: 200 }, (_, i) => { const x = 0.5 + i * 0.5;` |
...
5 black swans: methane clathrate release, AMOC shutdown, permafrost tipping cascade, multi-breadbasket failure, Antarctic ice sheet collapse. Each with probability estimate, portfolio impact, and hedging strategy.

**Standards:** ['EVT', 'Lenton et al. (2019)']
**Reference documents:** IPCC AR6 Tipping Elements (Chapter 4); Lenton et al. (2019) Tipping Cascades; Swiss Re Tail Risk Studies
