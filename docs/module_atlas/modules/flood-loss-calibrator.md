# Flood Loss Calibrator
**Module ID:** `flood-loss-calibrator` · **Route:** `/flood-loss-calibrator` · **Tier:** B (frontend-computed) · **EP code:** — · **Sprint:** —

## 2 · Function Map

### 2.1 Frontend (1 files)
**Components/functions:** `Badge`, `DEFAULT_MODEL`, `Kpi`, `STATES`
...
|---|---|---|
| `STATES` | 11 | `name` |
| `DEFAULT_MODEL` | 5 | `pct`, `modelled` |

**Derived values computed in the UI layer:**

| Variable | Expression |
|---|---|
| `usd` | `(v) => (v == null \|\| isNaN(v)) ? '—' : `$${Number(v).toLocaleString('en-US', { maximumFractionDigits: 0 })}`;` |
| `num` | `(v) => (v == null \|\| isNaN(v)) ? '—' : Number(v).toLocaleString('en-US');` |
| `modelRows` | `useMemo(() => model.map((m) => {` |
| `dev` | `(observed != null && m.modelled) ? ((m.modelled - observed) / observed) * 100 : null;` |
| `updateModel` | `(i, key, val) => setModel((prev) => prev.map((r, idx) => idx === i ? { ...r, [key]: Number(val) } : r));` |

## 3 · Data Sources & Provenance
**Provenance classes:** `frontend-seed`
**Frontend seed datasets:** `DEFAULT_MODEL`, `STATES`
