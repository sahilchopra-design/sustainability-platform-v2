...
| `waci` | `d.reduce((a,r)=>a+r.waci,0);const optWaci=d.reduce((a,r)=>a+r.optWaci,0);` |
| `sectorCarbon` | `useMemo(()=>SECTORS.map(s=>{const items=filtered.filter(r=>r.sector===s);return{name:s.length>10?s.slice(0,10)+'..':s,current:items.reduce((a,r)=>a+r.` |
...
| `scopeData` | `useMemo(()=>{if(!filtered.length)return[];return[{name:'Scope 1',value:filtered.reduce((a,r)=>a+r.scope1,0)},{name:'Scope 2',value:filtered.reduce((a,` |
