# Eu Compliance Cockpit
**Module ID:** `eu-compliance-cockpit` · **Route:** `/eu-compliance-cockpit` · **Tier:** B (frontend-computed) · **EP code:** — · **Sprint:** —

## 2 · Function Map

### 2.1 Frontend (1 files)
**Components/functions:** `API`, `Badge`, `Card`, `DEMO_RESULTS`, `DNSH_OBJECTIVES`, `ETS2_FUELS`, `EUDR_CERTIFICATIONS`, `EUDR_COMMODITIES`, `KpiCard`, `LiveBadge`, `MS_AREAS`, `NACE_OPTIONS`, `PassFail`, `STEPS`, `SectionH`

**Seed dataset schemas (record structure of each in-page dataset):**

| Dataset | Rows | Fields |
|---|---|---|
| `NACE_OPTIONS` | 8 | `label`, `evidenceKey`, `evidenceLabel`, `evidenceDefault`, `threshold` |
| `DNSH_OBJECTIVES` | 6 | `label` |
| `MS_AREAS` | 6 | `label` |
| `ETS2_FUELS` | 6 | `label` |
| `STEPS` | 7 | `short`, `title`, `reg`, `endpoint` |
...
| `leiValid` | `/^[A-Z0-9]{20}$/.test(profile.lei);` |
| `totalGhg` | `Number(profile.scope1) + Number(profile.scope2Loc) + Number(profile.scope3);` |
| `blob` | `new Blob([s5.result.ixbrl_html], { type: 'text/html' });` |
| `dnshPct` | `dnshVals.length ? (dnshVals.filter(d => d.met).length / dnshVals.length) * 100 : 0;` |
| `score` | `r.taxonomy_aligned ? 100 : Math.round(0.5 * (r.sc_score \|\| 0) + 0.3 * dnshPct + 0.2 * msScore);` |
| `overallScore` | `ranRows.length ? Math.round(ranRows.reduce((a, r) => a + r.score, 0) / ranRows.length) : null;` |

## 3 · Data Sources & Provenance
**Provenance classes:** `frontend-seed`
**Frontend seed datasets:** `DNSH_OBJECTIVES`, `ETS2_FUELS`, `EUDR_CERTIFICATIONS`, `EUDR_COMMODITIES`, `MS_AREAS`, `NACE_OPTIONS`, `STEPS`
