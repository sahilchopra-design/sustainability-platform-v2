# Ppa Xva Engine
**Module ID:** `ppa-xva-engine` · **Route:** `/ppa-xva-engine` · **Tier:** A (backend vertical) · **EP code:** — · **Sprint:** —

## 2 · Function Map

### 2.1 Frontend (1 files)
**Components/functions:** `Badge`, `DEFAULT_NETTING_CONTRACTS`, `Fld`, `Kpi`, `RATINGS`, `Slider`

**Seed dataset schemas (record structure of each in-page dataset):**

| Dataset | Rows | Fields |
|---|---|---|
| `DEFAULT_NETTING_CONTRACTS` | 3 | `fixed_price_usd_mwh`, `annual_volume_mwh`, `tenor_years`, `holder` |
...
| `setContract` | `(i, k, v) => setNetContracts((prev) => prev.map((c, j) => (j === i ? { ...c, [k]: v } : c)));` |

### 2.2 Backend endpoints
| Method | Path | Handler | Route file |
|---|---|---|---|
| POST | `/api/v1/ppa-xva/netting` | `compute_netting` | api/v1/routes/ppa_xva.py |
...
| Function | Args | Purpose |
|---|---|---|
| `RenewableProjectEngine.wind_yield` | turbine_class, region, num_turbines, wake_loss_pct, availability_pct, lat, lon | Calculate wind energy yield with P50/P75/P90 confidence levels. Resource input is the hand-authored per-region Weibull (k, lambda) table by default. OPTIONAL: if `lat`/`lon` are supplied, real NASA POWER measured/reanalysis mean wind speed (WS50M, averaged over the last full calendar year) is used instead -- location-specific rather than a regional bucket. NASA POWER only gives a mean speed, not a |
...
| `RenewableProjectEngine.solar_yield` | country, capacity_kwp, performance_ratio, degradation_pct_yr, lat, lon | Calculate solar energy yield with P50/P75/P90. Resource input is the hand-authored per-country GHI table by default. OPTIONAL: if `lat`/`lon` are supplied, real NASA POWER measured/reanalysis GHI (ALLSKY_SFC_SW_DWN, averaged over the last full calendar year and scaled to an annual total) is used instead -- location-specific rather than a single country-wide figure. Additive: the country-table path |
...
**Engine `renewable_project_engine` — reference constants / scoring weights:**
...
**Database tables:** `__future__` *(shared)*, `fastapi` *(shared)*, `published` *(shared)*, `pydantic` *(shared)*, `services` *(shared)*, `typing` *(shared)*
**Frontend seed datasets:** `DEFAULT_NETTING_CONTRACTS`, `RATINGS`

## 4 · End-to-End Data Lineage (source → transformation → UI)

### 4.2 Traced backend call chains (lineage harness)

**GET /api/v1/ppa-xva/ref/collateral-haircuts** — status `passed`, provenance ['computed'], source tables: —
Output: `{'type': 'object', 'keys': ['label', 'haircuts_pct'], 'n_keys': 2}`

**GET /api/v1/ppa-xva/ref/migration-matrix** — status `passed`, provenance ['computed'], source tables: —
Output: `{'type': 'object', 'keys': ['label', 'states', 'annual_transition_pct', 'row_sums_check', 'usage'], 'n_keys': 5}`

**GET /api/v1/ppa-xva/ref/pd-curves** — status `passed`, provenance ['computed'], source tables: —
Output: `{'type': 'object', 'keys': ['label', 'tenors_years', 'cumulative_pd_pct', 'interpolation', 'source_basis'], 'n_keys': 5}`

**GET /api/v1/ppa-xva/ref/sa-ccr-params** — status `passed`, provenance ['computed'], source tables: —
Output: `{'type': 'object', 'keys': ['label', 'alpha', 'alpha_basis', 'commodity_supervisory_factors_pct', 'sf_basis', 'maturity_factor', 'capital'], 'n_keys': 7}`

**POST /api/v1/ppa-xva/cva** — status `failed`, provenance ['computed'], source tables: —
Output: `None`

**POST /api/v1/ppa-xva/netting** — status `failed`, provenance ['computed'], source tables: —
Output: `None`
...
|---|---|
| `pf-credit-rating-engine` | table:published |
