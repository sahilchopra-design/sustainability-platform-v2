# Api::Openfema_Claims
**Module ID:** `api::openfema_claims` · **Route:** `/api/v1/openfema` · **Tier:** A (backend vertical) · **EP code:** — · **Sprint:** —
...
|---|---|---|---|
| GET | `/api/v1/openfema/states` | `valid_states` | api/v1/routes/openfema_claims.py |

## 3 · Data Sources & Provenance
**Provenance classes:** `computed`

**Database tables:** `__future__` *(shared)*, `exc` *(shared)*, `fastapi` *(shared)*, `severity`, `typing` *(shared)*

## 4 · End-to-End Data Lineage (source → transformation → UI)

### 4.2 Traced backend call chains (lineage harness)

**GET /api/v1/openfema/claims-summary** — status `failed`, provenance ['computed'], source tables: —
Output: `None`

**GET /api/v1/openfema/states** — status `passed`, provenance ['computed'], source tables: —
Output: `{'type': 'object', 'keys': ['count', 'states'], 'n_keys': 2}`
