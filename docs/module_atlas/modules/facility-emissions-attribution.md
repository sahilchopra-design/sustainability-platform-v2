# Facility Emissions Attribution
**Module ID:** `facility-emissions-attribution` · **Route:** `/facility-emissions-attribution` · **Tier:** B (frontend-computed) · **EP code:** — · **Sprint:** —

## 2 · Function Map

### 2.1 Frontend (1 files)
**Components/functions:** `API`, `BAR_COLORS`, `Badge`, `COUNTRIES`, `CT_API`, `DEMO_SAMPLE`, `KpiCard`, `SECTORS`, `SectionH`
...
| `CT_API` | ``${API}/api/v1/climate-trace`;` |
| `fmtMt` | `(n) => (n == null ? '—' : (n / 1e6).toFixed(2));` |
| `setOwn` | `(id, v) => setOwnership(prev => ({ ...prev, [id]: Math.max(0, Math.min(100, v)) }));` |
| `attributed` | `useMemo(() => facilities.reduce((a, f) => {` |
| `totalRaw` | `data.total_co2e_tyr \|\| facilities.reduce((a, f) => a + f.co2e_tyr, 0);` |
| `disclosedT` | `disclosedMt * 1e6;` |
| `gapT` | `attributed - disclosedT;` |
| `gapPct` | `disclosedT > 0 ? (gapT / disclosedT) * 100 : null;` |
| `topChart` | `useMemo(() => facilities.slice(0, 12).map(f => ({` |
| `pct` | `(ownership[f.id] ?? 100) / 100;` |

## 3 · Data Sources & Provenance
**Provenance classes:** `frontend-seed`
**Frontend seed datasets:** `BAR_COLORS`, `COUNTRIES`, `SECTORS`
