...
| `sorted` | `[...COUNTRY_FOOTPRINTS].sort((a, b) => b.per_capita_t - a.per_capita_t);` |
| `rows` | `pricedActivities.map(a => ({ Activity: a.activity, Carbon_kg: a.carbon_kg, Dollar_Cost: a.money_cost, Carbon_Cost: a.carbon_cost.toFixed(3), True_Cost` |
