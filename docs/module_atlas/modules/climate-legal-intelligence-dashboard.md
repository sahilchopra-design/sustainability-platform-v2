...
- Ingest case filings from Sabin Center, ClientEarth, and jurisdiction-specific court databases
- Classify by claim type (disclosure failure, greenwashing, stranded asset, human rights)
- Score entities by exposure using case weight, jurisdiction enforceability, and precedent strength
- Track case progression and update risk scores on outcome events
...
| `netLegalVaR` | `Math.max(0, pendingLitigationM * compositeLegalRisk / 100 - insuranceCoverageM * 0.8);` |
| `legalTeamSize` | `Math.floor(sr(i * 61) * 50 + 2);` |
| `externalCounsel` | `LEGAL_READINESS[Math.floor(sr(i * 67) * 3)];` |
| `legalReadinessScore` | `Math.round(sr(i * 71) * 70 + 20);` |
| `JUR_INTELLIGENCE` | `JURISDICTIONS_25.map((jur, ji) => {` |
| `legalMaturityScore` | `Math.round(sr(ji * 73 + 6000) * 70 + 20);` |
| `caseCount` | `Math.floor(sr(ji * 79 + 6000) * 80 + 5);` |
| `totalDamagesM` | `Math.round((sr(ji * 83 + 6000) * 5 + 0.1) * 1e3);` |
| `activePrecedents` | `Math.floor(sr(ji * 89 + 6000) * 20);` |
| `jurIdx` | `Math.floor(sr(p * 107 + 6500) * 25);` |
| `claimIdx` | `Math.floor(sr(p * 109 + 6500) * 6);` |
| `year` | `2005 + Math.floor(sr(p * 113 + 6500) * 19);` |
| `damagesM` | `Math.round((sr(p * 117 + 6500) * 2 + 0.01) * 1e3);` |
| `impactScore` | `Math.round(sr(p * 119 + 6500) * 80 + 15);` |
| `outcomes` | `['Plaintiff Win', 'Defendant Win', 'Settlement', 'Dismissed', 'Precedent-Setting'];` |
| `bau` | `Math.round(500 * Math.pow(1 + SCENARIO_CAGR[0], y));` |
| `top20` | `useMemo(() => [...ENTITIES].sort((a, b) => b.compositeLegalRisk - a.compositeLegalRisk).slice(0, 20), []);` |
| `riskDist` | `useMemo(() => [[0, 25], [25, 50], [50, 70], [70, 85], [85, 100]].map(([lo, hi]) => ({` |
| `weightedComposite` | `useMemo(() => { const totalWeight = Object.values(dimWeights).reduce((s, v) => s + v, 0);` |
| `weightedScore` | `Math.round(RISK_DIMENSIONS.reduce((s, d) => s + e[d] * dimWeights[d], 0) / totalWeight);` |
| `jurStats` | `useMemo(() => [...JUR_INTELLIGENCE].sort((a, b) => b.legalMaturityScore - a.legalMaturityScore), []);` |
| `sectorDimAvg` | `useMemo(() => SECTORS.map(s => {` |
...
Aggregates weighted case exposure across active filings, appeals, and enforcement notices for each entity.

**Standards:** ['Sabin Center Climate Case Chart', 'ClientEarth Case Database']
**Reference documents:** Sabin Center Global Climate Litigation Database 2024; UNEP Global Trends in Climate Change Litigation 2023; ClientEarth Strategic Litigation Report
