...
- Import bond term sheet or issuer details via the data entry panel
- Use-of-Proceeds tab classifies and scores each eligible category against ICMA/EU criteria
- Issuer ESG tab imports company profile scores and controversy flags
- Greenwashing Risk Assessment tab applies integrity scoring model
- EU Taxonomy tab verifies substantial contribution and DNSH for eligible activities
- Generate Credit Committee ESG Addendum Report with findings and recommendations
...
| Dataset | Rows | Fields |
|---|---|---|
| `CCP_DIMS` | 9 | `score`, `desc` |
| `GREENWASH_FLAGS` | 9 | `frequency`, `severity`, `type` |
| `INTEGRITY_TIERS` | 6 | `score`, `premium`, `volume`, `color` |
...
| `sortedCredits` | `useMemo(() => [...CREDITS].sort((a, b) => sortBy === 'overall' ? b.overall - a.overall : sortBy === 'price' ? b.price - a.price : b.additionality - a.additionality), [sortBy] );` |
...
Use-of-proceeds alignment scores each eligible category against ICMA GBP and EU Taxonomy Article 3 criteria. Process management evaluates project evaluation framework, ring-fencing of proceeds, and allocation reporting frequency. Issuer ESG score imports from Company Profiles module. Greenwashing risk flags trigger if use-of-proceeds score <40 or issuer controversy severity ≥4.

**Standards:** ['ICMA Green Bond Principles 2021', 'EU Green Bond Standard 2023', 'CBI Climate Bonds Standard']
**Reference documents:** ICMA Green Bond Principles 2021; EU Green Bond Standard Regulation (EU) 2023/2631; Climate Bonds Initiative Certification Criteria; EU Taxonomy Regulation (EU) 2020/852
