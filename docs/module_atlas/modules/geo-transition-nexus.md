...
- Combined Score Matrix shows transition vs geopolitical
- Fossil State Risk identifies vulnerable economies
...
| Dataset | Rows | Fields |
|---|---|---|
| `COUNTRIES` | 26 | `transition`, `geopolitical`, `fossil_rev_pct`, `portfolio_exp` |
| `POLICY_REVERSALS` | 7 | `scenario`, `probability`, `impact`, `transition_delta` |
...
| `combined` | `useMemo(() => COUNTRIES.map(c => { const w = geoWeight / 100;` |
...
Fossil state risk: countries where fossil revenue dependency + political instability = stranded state risk. Policy reversal: scenarios where government change reverses climate commitments.

**Standards:** ['Model', 'NGFS']
**Reference documents:** NGFS; V-Dem Institute
