...
MODULE_SIGNATURES: dict[str, dict] = {
    ...
    "double_materiality_engine": {
        "label": "Double Materiality (DMA) Engine",
        "category": "regulatory",
    ...
    # ---- Signatures for services referenced by MODULE_DEPENDENCIES that were
    # ---- never registered (referential-integrity gap found in E2E testing) ----
    "company_profiles": {
        "label": "Company Master Reference",
        "category": "cross_module",
        "inputs": ["exchange_listings", "brsr_submissions", "manual_curation"],
        "outputs": ["company_master_records", "sector_classification", "identifiers"],
        "reference_data": ["exchange_ticker_map", "nace_gics_crosswalk"],
        "quality_indicators": ["identifier_coverage_pct", "staleness_days"],
    },
    "csrd_kpi_store": {
        "label": "CSRD KPI Store",
        "category": "regulatory",
        "inputs": ["esrs_datapoint_values", "extraction_run_ids"],
        "outputs": ["kpi_time_series", "datapoint_snapshots"],
        "reference_data": ["esrs_datapoints_registry"],
        "quality_indicators": ["datapoint_fill_rate", "validation_pass_rate"],
    },
    "disclosure_completeness": {
        "label": "Disclosure Completeness Scorer",
        "category": "regulatory",
        "inputs": ["entity_disclosures", "framework_requirements"],
        "outputs": ["completeness_score", "gap_list", "framework_coverage"],
        "reference_data": ["framework_requirement_catalogs"],
        "quality_indicators": ["requirement_coverage_pct"],
    },
    "engagement_tracker": {
        "label": "Engagement Tracker",
        "category": "sustainability",
        "inputs": ["engagement_actions", "milestone_definitions", "voting_records"],
        "outputs": ["engagement_status", "escalation_flags", "milestone_progress"],
        "reference_data": ["engagement_playbooks"],
        "quality_indicators": ["action_logging_completeness"],
    },
    "epc_transition_engine": {
        "label": "EPC Transition Engine",
        "category": "real_estate",
        "inputs": ["epc_ratings", "retrofit_cost_curves", "mees_scenario"],
        "outputs": ["retrofit_pathways", "stranding_years", "epc_uplift_costs"],
        "reference_data": ["epc_register_extracts", "crrem_pathways"],
        "quality_indicators": ["epc_coverage_pct", "cost_curve_vintage"],
    },
    "gdelt_controversy": {
        "label": "GDELT Controversy Feed",
        "category": "sentiment",
        "inputs": ["gdelt_event_stream", "entity_alias_map"],
        "outputs": ["controversy_events", "severity_scores", "entity_mentions"],
        "reference_data": ["controversy_taxonomy"],
        "quality_indicators": ["entity_match_precision", "feed_latency_hours"],
    },
    "green_premium_engine": {
        "label": "Green Premium Engine",
        "category": "real_estate",
        "inputs": ["certified_asset_transactions", "certification_levels", "market_comparables"],
        "outputs": ["green_premium_bps", "brown_discount_bps", "tenant_demand_index"],
        "reference_data": ["certification_schemes", "transaction_evidence_panels"],
        "quality_indicators": ["comparable_sample_size"],
    },
    "peer_benchmark_engine": {
        "label": "Peer Benchmark Engine",
        "category": "cross_module",
        "inputs": ["entity_scores", "peer_group_definitions", "sector_maps"],
        "outputs": ["peer_percentiles", "quartile_rankings", "benchmark_gaps"],
        "reference_data": ["peer_group_registry"],
        "quality_indicators": ["peer_group_size", "score_coverage_pct"],
    },
    "portfolio_analytics": {
        "label": "Portfolio Analytics Engine",
        "category": "cross_module",
        "inputs": ["portfolio_holdings", "entity_scores", "market_data"],
        "outputs": ["portfolio_esg_scores", "attribution_results", "concentration_metrics"],
        "reference_data": ["benchmark_constituents"],
        "quality_indicators": ["holdings_coverage_pct"],
    },
    "portfolio_analytics_engine_v2": {
        "label": "Portfolio Analytics Engine v2",
        "category": "cross_module",
        "inputs": ["portfolio_holdings", "sovereign_scores", "scenario_results"],
        "outputs": ["alignment_gap", "portfolio_temperature", "scenario_pnl"],
        "reference_data": ["benchmark_pathways"],
        "quality_indicators": ["holdings_coverage_pct", "pathway_vintage"],
    },
    "portfolio_health_engine": {
        "label": "Portfolio Health Engine",
        "category": "dme",
        "inputs": ["dme_velocity_signals", "dme_alerts", "portfolio_holdings"],
        "outputs": ["portfolio_health_index", "watchlist", "deterioration_flags"],
        "reference_data": ["health_index_weights"],
        "quality_indicators": ["signal_freshness_hours"],
    },
    "re_clvar_engine": {
        "label": "Real Estate Climate VaR Engine",
        "category": "real_estate",
        "inputs": ["property_locations", "hazard_layers", "nature_dependency_scores", "asset_values"],
        "outputs": ["property_climate_var", "hazard_loss_curves", "portfolio_clvar"],
        "reference_data": ["spatial_hazard_layers", "damage_functions"],
        "quality_indicators": ["geocoding_precision", "hazard_layer_vintage"],
    },
    "stress_test_runner": {
        "label": "Stress Test Runner",
        "category": "scenarios",
        "inputs": ["scenario_definitions", "contagion_shocks", "portfolio_exposures"],
        "outputs": ["stressed_pnl", "stressed_capital_ratios", "breach_reports"],
        "reference_data": ["ngfs_scenario_parameters"],
        "quality_indicators": ["scenario_coverage", "run_reproducibility"],
    },
    "supply_chain_scope3_engine": {
        "label": "Supply Chain Scope 3 Engine",
        "category": "emissions",
        "inputs": ["supplier_map", "spend_data", "eeio_factors", "commodity_flows"],
        "outputs": ["scope3_estimates", "tier_emissions", "hotspot_suppliers"],
        "reference_data": ["eeio_factor_tables", "commodity_emission_factors"],
        "quality_indicators": ["supplier_data_share", "estimation_share_pct"],
    },
    "carbon_price_ets_engine": {
        "label": "Carbon Price & ETS Engine",
        "category": "emissions",
        "inputs": ["ets_auction_data", "policy_signals", "scenario_price_paths"],
        "outputs": ["carbon_price_curves", "ets_exposure_costs", "price_forecasts"],
        "reference_data": ["eu_ets_history", "ngfs_carbon_price_paths"],
        "quality_indicators": ["price_feed_freshness"],
    },
}
...
MODULE_DEPENDENCIES: list[dict] = [
    ...
    {"source": "pcaf_waci_engine", "target": "regulatory_report_compiler",
    ...
    {"source": "csrd_auto_populate", "target": "eu_taxonomy_engine",
    ...
    {"source": "csrd_auto_populate", "target": "transition_plan_engine",
    ...
    {"source": "csrd_auto_populate", "target": "double_materiality_engine",
    ...
    {"source": "double_materiality_engine", "target": "csrd_auto_populate",
    ...
    {"source": "nature_risk_calculator", "target": "sfdr_pai_engine",
    ...
    {"source": "dme_greenwashing_engine", "target": "entity360",
    ...
    {"source": "dme_nlp_pulse_engine", "target": "entity360",
    ...
    {"source": "dme_policy_tracker_engine", "target": "carbon_price_ets_engine",
    ...
    {"source": "sentiment_analysis_engine", "target": "ecl_climate_engine",
    ...
    {"source": "sentiment_analysis_engine", "target": "pe_deal_engine",
    ...
    {"source": "sentiment_analysis_engine", "target": "real_estate_valuation_engine",
