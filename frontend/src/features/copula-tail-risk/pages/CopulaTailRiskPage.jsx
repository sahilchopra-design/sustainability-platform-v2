...
export default function CopulaTailRiskPage(){
  ...
  const kpis=useMemo(()=>{const n=filtered.length||1;return{count:filtered.length,avgVaR:(filtered.reduce((s,p)=>s+parseFloat(p.var95),0)/n).toFixed(2),avgCVaR:(filtered.reduce((s,p)=>s+parseFloat(p.cvar95),0)/n).toFixed(2),avgTail:(filtered.reduce((s,p)=>s+parseFloat(p.tailIndex),0)/n).toFixed(2),avgDD:(filtered.reduce((s,p)=>s+parseFloat(p.maxDD),0)/n).toFixed(2)};},[filtered]);
