          ...
          <div style={{ background: T.card, borderRadius: 8, border: `1px solid ${T.border}`, padding: 20, marginTop: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
              {[
                ...
                { label: 'Avg Climate Mort Rate', value: (COUNTRIES.length ? COUNTRIES.reduce((s, c) => s + calcClimateMortRate(c, scenIdx), 0) / COUNTRIES.length * 1000 : 0).toFixed(1) + '‰', color: T.red },
                { label: 'Avg SMR', value: (COUNTRIES.length ? COUNTRIES.reduce((s, c) => s + calcSMR(c, scenIdx), 0) / COUNTRIES.length : 0).toFixed(3), color: T.orange },
                { label: 'Avg Net Longevity', value: (COUNTRIES.length ? COUNTRIES.reduce((s, c) => s + calcNetLongevity(c, scenIdx), 0) / COUNTRIES.length : 0).toFixed(2) + '%', color: T.green },
                { label: 'Avg Life Exp', value: (COUNTRIES.length ? COUNTRIES.reduce((s, c) => s + calcLifeExp(c, scenIdx), 0) / COUNTRIES.length : 0).toFixed(1) + ' yrs', color: T.navy },
                { label: 'Avg PVFB $M', value: '$' + (COUNTRIES.length ? COUNTRIES.reduce((s, c) => s + calcPVFB(c, scenIdx), 0) / COUNTRIES.length : 0).toFixed(0) + 'M', color: T.purple },
