...
export default function ReinsuranceClimatePage(){
  ...
  const renderILS=()=>(
    <div>
      <div style={S.grid4}>
        {[{l:'Outstanding Bonds',v:CAT_BONDS.filter(b=>b.status==='Outstanding').length},{l:'Market Size',v:`$${(CAT_BONDS.filter(b=>b.status==='Outstanding').reduce((a,b)=>a+b.size,0)/1000).toFixed(1)}B`},{l:'Avg Coupon',v:`${(CAT_BONDS.reduce((a,b)=>a+b.coupon,0)/CAT_BONDS.length).toFixed(1)}%`},{l:'Avg Spread',v:`${(CAT_BONDS.reduce((a,b)=>a+b.spread,0)/CAT_BONDS.length).toFixed(1)}%`},{l:'Avg Exp. Loss',v:`${(CAT_BONDS.reduce((a,b)=>a+b.expectedLoss,0)/CAT_BONDS.length).toFixed(1)}%`},{l:'Climate Premium',v:`${(CAT_BONDS.reduce((a,b)=>a+b.climateRiskPremium,0)/CAT_BONDS.length).toFixed(1)}%`},{l:'Parametric Triggers',v:CAT_BONDS.filter(b=>b.triggerType==='Parametric').length},{l:'Matured/Paid',v:CAT_BONDS.filter(b=>b.status==='Matured').length}].map((k,i)=>(
