...
export default function GovernanceHubPage(){
  ...
  const kpis=useMemo(()=>{if(!filtered.length)return{count:0,avgGov:'0.0',avgWomen:'0.0',avgIndep:'0.0',sepChairs:0};return{count:filtered.length,avgGov:(filtered.reduce((s,c)=>s+parseFloat(c.govScore),0)/filtered.length).toFixed(1),avgWomen:(filtered.reduce((s,c)=>s+parseFloat(c.womenPct),0)/filtered.length).toFixed(1),avgIndep:(filtered.reduce((s,c)=>s+parseFloat(c.indepPct),0)/filtered.length).toFixed(1),sepChairs:filtered.filter(c=>c.sepChair).length};},[filtered]);
