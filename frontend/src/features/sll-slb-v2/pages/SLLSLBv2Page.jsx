...
function Tab1({instrumentType,issuerName,sector,notional,kpiName}) {
  ...
  const run=useCallback(async()=>{ setLoading(true); setError(null); try { const r=await axios.post((process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001') + '/api/v1/sll-slb-v2/assess',{instrument_type:instrumentType,issuer_name:issuerName,sector,notional_usd:parseFloat(notional),kpi_name:kpiName}); setResult(r.data); } catch { setError('API unavailable — demo mode.'); setResult({}); } setLoading(false); },[instrumentType,issuerName,sector,notional,kpiName]);
  const scores=[
