    ...
    reader.onload = (ev) => {
      try {
        ...
        if (Array.isArray(imported)) {
          const customs = imported.filter(t => !DEFAULT_TEMPLATES.some(d => d.id === t.id)).map(t => ({ ...t, isDefault: false, id: t.id || `imported_${Date.now()}_${Math.random().toString(36).slice(2, 6)}` }));
