...
export default function NatureLossRiskPage(){
  ...
  const kpis=useMemo(()=>{const avg=(k)=>Math.round(COMPANIES.reduce((s,c)=>s+c[k],0)/ Math.max(1, COMPANIES.length));return{avgDep:avg('natureDep'),avgImpact:avg('natureImpact'),avgTnfd:avg('tnfdReadiness'),committed:COMPANIES.filter(c=>c.sbtNature==='Committed').length,avgMsa:+(COMPANIES.reduce((s,c)=>s+c.msa,0)/ Math.max(1, COMPANIES.length)).toFixed(3)};},[]);
  ...
  const radarData=useMemo(()=>{const dims=['natureDep','natureImpact','tnfdReadiness','leapScore','waterDep','supplyChainNature'];const avg=(k)=>Math.round(COMPANIES.reduce((s,c)=>s+c[k],0)/ Math.max(1, COMPANIES.length));return dims.map(d=>({dim:d.replace(/([A-Z])/g,' $1').trim(),value:avg(d),fullMark:100}));},[]);
