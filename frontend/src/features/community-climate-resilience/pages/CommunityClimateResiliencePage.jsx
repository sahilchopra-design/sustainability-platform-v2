...
export default function CommunityClimateResiliencePage() {
  ...
  const kpis = [
    ...
    { label: '% Indigenous Communities', value: `${indigenousPct.toFixed(1)}%`, color: PURPLE },
  ...
  return (
    <div style={{ background: T.bg, minHeight: '100vh', padding: '24px', fontFamily: 'DM Sans, sans-serif' }}>
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        ...
        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 8, padding: 16, marginBottom: 20, display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          {[
          ...
          <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 8, padding: 20 }}>
            ...
            <ResponsiveContainer width="100%" height={380}>
              <ScatterChart margin={{ left: 0 }}>
                ...
                <Tooltip cursor={{ strokeDasharray: '3 3' }} content={({ payload }) => payload?.length ? <div style={{ background: T.surface, border: `1px solid ${T.border}`, padding: '8px 12px', fontSize: 12 }}><b>{payload[0]?.payload?.name}</b><br />Funding: ${payload[0]?.payload?.x}M<br />Vulnerability: {payload[0]?.payload?.y}</div> : null} />
          ...
          <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 8, padding: 20 }}>
            ...
            <ResponsiveContainer width="100%" height={380}>
              <BarChart data={[
                ...
                <Bar yAxisId="left" dataKey="score" fill={PURPLE} radius={[3, 3, 0, 0]} name="Avg Resilience Score" />
          ...
          <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 8, padding: 20 }}>
            ...
            <ResponsiveContainer width="100%" height={380}>
              <BarChart data={['Coastal', 'Inland'].map(type => {
                ...
                <Bar yAxisId="right" dataKey="funding" fill={BLUE} radius={[3, 3, 0, 0]} name="Avg Funding ($M)" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
            ...
            <tbody>
              {filtered.slice(0, 25).map(c => (
                <tr key={c.id} style={{ borderBottom: `1px solid ${T.borderL}` }}>
                  ...
                  <td style={{ padding: '7px 10px', color: T.text }}>{c.populationK}</td>
                  <td style={{ padding: '7px 10px' }}><span style={{ background: T.surfaceH, padding: '2px 8px', borderRadius: 10, fontSize: 11 }}>{c.incomeLevel}</span></td>
                  ...
                  <td style={{ padding: '7px 10px', color: c.indigenousCommunity ? PURPLE : T.textSec, fontWeight: c.indigenousCommunity ? 600 : 400 }}>{c.indigenousCommunity ? 'Yes' : 'No'}</td>
                  <td style={{ padding: '7px 10px', color: c.coastalExposure ? BLUE : T.textSec, fontWeight: c.coastalExposure ? 600 : 400 }}>{c.coastalExposure ? 'Yes' : 'No'}</td>
                </tr>
              ))}
            </tbody>
          </table>
