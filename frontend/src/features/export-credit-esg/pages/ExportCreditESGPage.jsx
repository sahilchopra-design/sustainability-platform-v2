...
function Tab1({exporterCountry,sector,ecaName}) {
  ...
  const run = useCallback(async()=>{ setLoading(true); setError(null); try { const r=await axios.post((process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001') + '/api/v1/export-credit-esg/assess',{exporter_country:exporterCountry,sector,eca_name:ecaName}); setResult(r.data); } catch { setError('API unavailable — demo mode.'); setResult({}); } setLoading(false); },[exporterCountry,sector,ecaName]);
  return (
    <div>
