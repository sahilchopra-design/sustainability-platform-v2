...
function Tab1({technology,stage,geography}) {
  ...
  const run=useCallback(async()=>{ setLoading(true); setError(null); try { const r=await axios.post((process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001') + '/api/v1/climate-tech/assess-technology',{technology,stage,geography}); setResult(r.data); } catch { setError('API unavailable — demo mode.'); setResult({}); } setLoading(false); },[technology,stage,geography]);
  return (
    <div>
