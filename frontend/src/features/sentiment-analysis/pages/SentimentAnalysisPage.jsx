...
function Tab1({entityName,sector}) {
  ...
  const run = useCallback(async()=>{ setLoading(true); setError(null); try { const r=await axios.post((process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001') + '/api/v1/sentiment-analysis/assess',{entity_name:entityName,sector}); setResult(r.data); } catch { setError('API unavailable — demo mode.'); setResult({}); } setLoading(false); },[entityName,sector]);
  return (
    <div>
