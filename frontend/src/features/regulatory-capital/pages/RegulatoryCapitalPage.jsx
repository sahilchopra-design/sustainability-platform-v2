...
function Tab1({institutionType, totalAssets, approach}) {
  ...
  const run = useCallback(async()=>{
    ...
    try { const r=await axios.post((process.env.REACT_APP_BACKEND_URL || process.env.REACT_APP_API_URL || 'http://localhost:8001') + '/api/v1/regulatory-capital/calculate-ratios',{institution_type:institutionType,total_assets_eur_bn:totalAssets,approach}); setResult(r.data); }
...
function Tab2({approach}) {
  ...
  const run = useCallback(async()=>{
    setLoading(true); setError(null);
    try { await axios.post((process.env.REACT_APP_BACKEND_URL || process.env.REACT_APP_API_URL || 'http://localhost:8001') + '/api/v1/regulatory-capital/calculate-ratios',{approach}); }
