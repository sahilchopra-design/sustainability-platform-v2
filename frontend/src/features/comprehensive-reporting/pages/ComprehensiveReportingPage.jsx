...
function Tab1({entityName,frameworks,reportingYear}) {
  ...
  const run=useCallback(async()=>{ setLoading(true); setError(null); try { const r=await axios.post((process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001') + '/api/v1/comprehensive-reporting/compile',{entity_name:entityName,frameworks,reporting_year:parseInt(reportingYear)}); setResult(r.data); } catch { setError('API unavailable — demo mode.'); setResult({}); } setLoading(false); },[entityName,frameworks,reportingYear]);
  return (
    <div>
