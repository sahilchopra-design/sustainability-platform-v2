...
function Tab1({entityType,jurisdiction}) {
  ...
  const run=useCallback(async()=>{ setLoading(true); setError(null); try { const r=await axios.post((process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001') + '/api/v1/regulatory-horizon/scan',{entity_type:entityType,jurisdiction,horizon_years:5}); setResult(r.data); } catch { setError('API unavailable — demo mode.'); setResult({}); } setLoading(false); },[entityType,jurisdiction]);
  return (
    <div>
