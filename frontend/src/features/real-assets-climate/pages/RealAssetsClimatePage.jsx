...
export default function RealAssetsClimatePage(){
  ...
  const yearDist=useMemo(()=>{const m={};[...ASSETS].sort((a,b)=>a.strandingYear-b.strandingYear).forEach(a=>{const bucket=a.strandingYear<=2030?'By 2030':a.strandingYear<=2035?'2031-2035':a.strandingYear<=2040?'2036-2040':'After 2040';m[bucket]=(m[bucket]||0)+1;});return Object.entries(m).map(([name,value])=>({name,value}));},[]);
  ...
  const renderStrand=()=>{const strandData=[...ASSETS].sort((a,b)=>a.strandingYear-b.strandingYear);
