...
function Tab1({entityName,sector,supplyCountries}) {
  ...
  const run=useCallback(async()=>{ setLoading(true); setError(null); try { const r=await axios.post((process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001') + '/api/v1/modern-slavery/assess',{entity_name:entityName,sector,primary_supply_countries:supplyCountries}); setResult(r.data); } catch { setError('API unavailable — demo mode.'); setResult({}); } setLoading(false); },[entityName,sector,supplyCountries]);
  return (
    <div>
