  ...
  const enriched = useMemo(() => {
    ...
    return portfolio.map((p, idx) => {
      ...
      GRESB_ASPECTS.forEach(a => {
        const score = overrides[a.id] !== undefined ? overrides[a.id] : (gs[a.id] !== undefined ? gs[a.id] : Math.round(8 + Math.abs(Math.sin(idx * 7 + GRESB_ASPECTS.indexOf(a) * 3)) * 12));
      ...
      const baseHistory = p.gresb_history || Array.from({ length: 5 }, (_, i) => ({
        year: 2022 + i,
        score: Math.max(30, Math.min(100, totalScore - (4 - i) * (3 + Math.round(Math.sin(idx + i) * 2)))),
      }));
